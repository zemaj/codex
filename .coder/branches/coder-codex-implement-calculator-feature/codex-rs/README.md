...
Evaluate arithmetic expressions from the CLI or inside the TUI.

- CLI: `codex calc "1 + 2*3"` → prints `7`
- Control precision: `codex calc -p 4 "2/3"` → `0.6667`
- Raw (no trimming): `codex calc --raw -p 6 "2/3"` → `0.666667`
- TUI: type `/calc 2^3^2` to show `= 512` inline.

Notes: calculations use deterministic `f64` math. Division by zero and syntax
errors are reported with a concise message that includes a byte position.
