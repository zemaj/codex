...
pub(crate) enum HistoryCell {
    ...
    /// Output from the `/calc` command.
    CalcOutput { view: TextBlock },
...
impl HistoryCell {
    ...
    pub(crate) fn new_calc_output(result_line: String) -> Self {
        // Keep it concise: just show a single result line and a blank separator.
        let lines: Vec<Line<'static>> = vec![Line::from(result_line), Line::from("")];
        HistoryCell::CalcOutput {
            view: TextBlock::new(lines),
        }
    }
