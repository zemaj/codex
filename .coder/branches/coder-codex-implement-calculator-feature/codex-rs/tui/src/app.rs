        ...
        while let Ok(event) = self.app_event_rx.recv() {
            match event {
                ...
                AppEvent::DispatchCommand(command, command_text) => match command {
                    ...
                    SlashCommand::Calc => {
                        if let AppState::Chat { widget } = &mut self.app_state {
                            let expr = command_text
                                .strip_prefix("/calc")
                                .unwrap_or(&command_text)
                                .trim()
                                .to_string();
                            if expr.is_empty() {
                                widget.add_calc_output("calc error: empty".to_string());
                            } else {
                                let opts = codex_core::calc::EvalOptions::default();
                                match codex_core::calc::evaluate_with_opts(&expr, &opts) {
                                    Ok(v) => {
                                        let s = codex_core::calc::format_value(
                                            v,
                                            opts.precision,
                                            opts.raw,
                                        );
                                        widget.add_calc_output(format!("= {}", s));
                                    }
                                    Err(e) => {
                                        widget.add_calc_output(format!("calc error: {:?}", e));
                                    }
                                }
                            }
                        }
