...
impl ChatWidget<'_> {
    ...
    pub(crate) fn submit_text_message(&mut self, text: String) {
        ...
        if text.starts_with("/calc") {
            let expr = text.strip_prefix("/calc").unwrap_or("").trim();
            let opts = codex_core::calc::EvalOptions {
                precision: 6,
                raw: false,
            };
            let result = codex_core::calc::evaluate_with_opts(expr, &opts);
            self.add_to_history(HistoryCell::new_user_prompt(text.clone()));
            match result {
                Ok(value) => {
                    let result_str = codex_core::calc::format_value(value, opts.precision, opts.raw);
                    self.add_to_history(HistoryCell::new_calc_result(format!("= {}", result_str)));
                }
                Err(err) => {
                    self.add_to_history(HistoryCell::new_error_event(format!("calc error: {}", err)));
                }
            }
        } else {
