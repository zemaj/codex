...
impl ChatWidget<'_> {
    ...
    pub(crate) fn handle_calc_command(&mut self, command_text: String) {
        use codex_core::calc::{evaluate, format_value};
        
        // Extract expression from command text (remove /calc prefix)
        let parts: Vec<&str> = command_text.trim().split_whitespace().collect();
        if parts.len() <= 1 {
            // No expression provided
            let error_msg = "calc error: no expression provided".to_string();
            self.add_to_history(HistoryCell::new_calc_output(error_msg));
            return;
        }
        
        // Join everything after /calc as the expression
        let expr = parts[1..].join(" ");
        
        match evaluate(&expr) {
            Ok(result) => {
                let formatted = format_value(result, 6, false);
                let output = format!("= {}", formatted);
                self.add_to_history(HistoryCell::new_calc_output(output));
            }
            Err(e) => {
                let error_msg = format!("calc error: {}", e);
                self.add_to_history(HistoryCell::new_calc_output(error_msg));
            }
        }
    }
