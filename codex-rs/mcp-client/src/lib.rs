...
//!   1. Spawning a subprocess (typically `codex-mcp-server`) whose STDIN/STDOUT
//!      transports newline-delimited JSON-RPC messages.
...
impl McpClient {
    ...
    pub async fn new_stdio_client(args: Vec<String>) -> std::io::Result<Self> {
        ...
        // Because we have invoked take() on both stdin and stdout, calling
        // `child.wait()` will not close the pipes now owned by tokio tasks.
        // We invoke `wait()` proactively to ensure the child process is reaped.
    ...
    async fn dispatch_response(
        ...
        let id = match resp.id {
            ...
            RequestId::String(_) => {
                ...
                debug!("response with string ID - no matching pending request");
                return;
            }
        };
        ...
        if let Some(tx) = pending.lock().await.remove(&id) {
            ...
            debug!(id, "no pending request found for response");
        }
    }
