...
// Create a custom enum for use with `CodexToolCallApprovalPolicy` where we
// intentionally exclude docstrings from the generated schema because they
// introduce anyOf in the the generated JSON schema, which makes it complex
...
impl CodexToolCallParam {
    pub fn into_config(self) -> std::io::Result<codex_core::config::Config> {
        let sandbox_policy = self.sandbox_permissions.map(|perms| {
        ...
        let overrides = codex_core::config::ConfigOverrides {
            ...
            approval_policy: self.approval_policy.map(Into::into),
            sandbox_policy,
