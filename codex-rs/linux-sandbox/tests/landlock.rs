...
use codex_core::exec::create_linux_sandbox_command_args;
...
async fn run_cmd(cmd: &[&str], writable_roots: &[PathBuf], _timeout_ms: u64) {
    let cwd = std::env::current_dir().expect("cwd should exist");
    let sandbox_policy = SandboxPolicy::new_read_only_policy_with_writable_roots(writable_roots);
    let full_args = create_linux_sandbox_command_args(
        cmd.iter().map(|c| c.to_string()).collect::<Vec<String>>(),
        &sandbox_policy,
        &cwd,
    );
    let program = env!("CARGO_BIN_EXE_codex-linux-sandbox");
    eprintln!("Running: {} {:?}", program, full_args);
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let result =
        process_exec_tool_call(params, SandboxType::LinuxSeccomp, ctrl_c, &sandbox_policy).await;
