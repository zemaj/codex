...
macro_rules! configure_command {
    (
        ...
        // Previously we called `env_clear()` followed by `envs(&env_map)` so
        // that the spawned process inherited *only* the variables explicitly
        // provided by the caller. This proved to be too restrictive for some
        // commands that expect important variables (such as `PATH`) to be
        // present.  Replace that strategy with a more surgical approach that
        // leaves the existing environment intact while still ensuring the
        // values in `env_map` take precedence.
