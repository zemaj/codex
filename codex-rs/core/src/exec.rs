...
pub async fn exec(params: ExecParams, ctrl_c: Arc<Notify>) -> Result<RawExecToolCallOutput> {
    let timeout_ms = params.timeout_ms;
    let child = spawn_child(params).await?;
    consume_truncated_output(child, ctrl_c, timeout_ms).await
}

/// Spawns the appropriate child process for the ExecParams.
async fn spawn_child(params: ExecParams) -> std::io::Result<Child> {
    let ExecParams {
        command,
        cwd,
        timeout_ms: _,
