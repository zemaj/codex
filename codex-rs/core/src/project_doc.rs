...
mod tests {
    ...
    /// the maximum number of bytes to embed from AGENTS.md.
    fn make_config(root: &TempDir, limit: usize) -> Config {
    ...
    async fn no_doc_file_returns_none() {
        ...
        let res = find_project_doc(&make_config(&tmp, 4096)).await.unwrap();
    ...
    async fn doc_smaller_than_limit_is_returned() {
        ...
        let res = find_project_doc(&make_config(&tmp, 4096))
            .await
            .unwrap()
            .expect("doc expected");
    ...
    async fn doc_larger_than_limit_is_truncated() {
        ...
        let res = find_project_doc(&make_config(&tmp, LIMIT))
            .await
            .unwrap()
            .expect("doc expected");
    ...
    async fn finds_doc_in_repo_root() {
        ...
        let mut cfg = make_config(&repo, 4096);
        cfg.cwd = nested;

        let res = find_project_doc(&cfg).await.unwrap().expect("doc expected");
    ...
    async fn zero_byte_limit_disables_docs() {
        ...
        let res = find_project_doc(&make_config(&tmp, 0)).await.unwrap();
        assert!(
            res.is_none(),
