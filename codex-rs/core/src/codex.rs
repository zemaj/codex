...
impl Session {
    ...
    fn record_rollout_items(&self, items: &[ResponseItem]) {
        let mut guard = self.rollout.lock().unwrap();
        if let Some(recorder) = guard.as_mut() {
            if let Err(e) = recorder.record_items(items) {
    ...
    loop {
        ...
        match sub.op {
            ...
            Op::ConfigureSession {
                ...
                let rollout_recorder = match RolloutRecorder::new(instructions.clone()) {
                    Ok(r) => Some(r),
                    Err(e) => {
...
async fn run_task(sess: Arc<Session>, sub_id: String, input: Vec<InputItem>) {
    ...
    loop {
        ...
        sess.record_rollout_items(&turn_input);
        ...
        match run_turn(&sess, sub_id.clone(), turn_input).await {
            Ok(turn_output) => {
                ...
                if !items.is_empty() {
                    ...
                    sess.record_rollout_items(&items);
