...
async fn run_turn(
    ...
    // Fetch external tools only for the first turn.
    let extra_tools = if is_first_turn {
        match sess.mcp.list_all_tools().await {
            Ok(v) => v,
            Err(e) => {
                tracing::warn!("failed to list tools from MCP servers: {e:#}");
                HashMap::new()
            }
        }
    } else {
