...
pub fn try_parse_word_only_commands_sequence(tree: &Tree, src: &str) -> Option<Vec<Vec<String>>> {
    ...
    const DISALLOWED_PUNCT_TOKENS: &[&str] = &["(", ")", "{", "}", "`", ">", ">>", "<"];
    ...
    while let Some(node) = stack.pop() {
        let kind = node.kind();
        if DISALLOWED_KINDS.contains(&kind) {
            return None;
        }
        if !node.is_named() {
            if DISALLOWED_PUNCT_TOKENS.contains(&kind) {
                return None;
            }
            const SAFE_OPERATOR_TOKENS: &[&str] = &["&&", "||", ";", "|"];
            if kind.chars().any(|c| "&;|".contains(c)) && !SAFE_OPERATOR_TOKENS.contains(&kind) {
                return None;
            }
            if ["(", ")", "{", "}"].contains(&kind) {
                return None;
