    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        match self {
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                // is per-user on macOS, so writes to TMPDIR should not be
                // readable by other users on the system.
                //
                // By comparison, TMPDIR is not guaranteed to be defined on
                // Linux or Windows, but supporting it here gives users a way
                // to provide the model with their own temporary directory
                // without having to hardcode it in the config.
