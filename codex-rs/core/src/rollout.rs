...
/// every update.
///
/// The actual file I/O is performed in a dedicated background thread so callers
/// can safely invoke [`RolloutRecorder::record_items`] while holding a lock.
///
/// We employ a *Tokio* mpsc channel to bridge between the async world and the
/// blocking writer thread. The sender side (`tx`) is `Clone` so it can be
/// shared freely; `record_items` is therefore an `async fn` that awaits the
/// `send` operation (which yields when the channel’s buffer is full) without
