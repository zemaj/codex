...
pub struct Prompt {
    ...
    /// Additional tools sourced from external MCP servers. Filled in only for
    /// the first turn. Note the key is the "fully qualified" tool name
    /// (i.e., prefixed with the server name), which should be reported to the
    /// model in place of Tool::name.
