...
mod tests {
    ...
    fn bash_lc_safe_examples() {
        ...
        // Multiple simple commands combined with allowed operators.
        assert!(is_known_safe_command(&vec_str(&[
            "bash",
            "-lc",
            "ls && pwd"
    ...
    fn bash_lc_unsafe_examples() {
        ...
        assert!(
            ...
            "Parentheses (subshell) should be rejected"
        );
        assert!(
            !is_known_safe_command(&vec_str(&["bash", "-lc", "ls || (pwd && echo hi)"])),
            "Nested parentheses should be rejected"
        );
    ...
    // (tests for bash helper parsing functions live in crate::bash)
}
