...
def main() -> None:
    ...
    with httpd:
        eprint(f"Starting local login server on http://localhost:{REQUIRED_PORT}/")
...
class _ApiKeyHTTPHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 – required by BaseHTTPRequestHandler
        ...
        if path == "/success":
            ...
            sys.exit(self.server.exit_code)
    ...
    def _exchange_code_for_api_key(self, code: str) -> str:
        ...
        with urllib.request.urlopen(
            ...
            api_key = exchange_payload["access_token"]

        # Persist refresh_token/id_token for future use (redeem credits etc.)
        # Not strictly necessary here, but included for parity with TS.
        auth_extra = {
            "tokens": {
                "id_token": id_token,
        ...
        # Merge into existing auth file once the key is written.
        self._auth_extra = auth_extra  # type: ignore[attr-defined]
...
def _write_auth_file(api_key: str, codex_home: str) -> bool:
    ...
    *stderr* so that the Rust layer can surface the problem.
    ...
    try:
        with open(auth_path, "w", encoding="utf-8") as fp:
            json.dump({"OPENAI_API_KEY": api_key}, fp)
...
LOGIN_SUCCESS_HTML = """const LOGIN_SUCCESS_HTML = String.raw`
<html lang="en">
  <head>
    <meta charset="utf-8" />
