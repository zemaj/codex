...
Currently, customers whose accounts are set to use Zero Data Retention (ZDR), they must set `disable_response_storage` to `true` so that Codex uses an alternative to the Responses API:
