...
pub(crate) fn append_markdown(
    ...
    let processed_markdown: std::borrow::Cow<'_, str> = if let Some(scheme) = config.file_opener {
        std::borrow::Cow::Owned(rewrite_file_citations(markdown_source, scheme, &config.cwd))
    } else {
