...
fn rewrite_file_citations(src: &str, file_opener: UriBasedFileOpener, cwd: &Path) -> String {
    ...
    CITATION_REGEX
        .replace_all(src, |caps: &regex::Captures<'_>| {
            ...
            format!("[{file}]({scheme}://file{absolute_path}:{start_line}) ")
        })
        .into_owned()
}
...
mod tests {
    ...
    fn citation_is_rewritten_with_relative_path() {
        ...
        assert_eq!(
            "Refer to [lib/mod.rs](windsurf://file/home/user/project/lib/mod.rs:5)  here.",
            result
        );
    }
    ...
    fn citation_followed_by_space_so_they_do_not_run_together() {
        ...
        assert_eq!(
            "TODOs on lines [src/foo.rs](vscode://file/home/user/project/src/foo.rs:24) [src/foo.rs](vscode://file/home/user/project/src/foo.rs:42) ",
            result
        );
    }
