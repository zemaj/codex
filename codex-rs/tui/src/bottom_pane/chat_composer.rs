...
impl ChatComposer<'_> {
    ...
    fn sync_command_popup(&mut self) {
        ...
        if first_line.starts_with('/') {
            ...
            let popup = self
                .command_popup
                .get_or_insert_with(CommandPopup::new);
    ...
    fn handle_key_event_with_popup(&mut self, key_event: KeyEvent) -> (InputResult, bool) {
        ...
        match key_event.into() {
            ...
            Input {
                ...
                if let Some(cmd) = popup.selected_command() {
                    // TODO: actually run command instead of submitting it to the model.
                    let _command_text = format!("/{}", cmd.command());
                    self.textarea.select_all();
                    self.textarea.cut();
                    // Hide popup since command has been handled.
                    self.command_popup = None;
    ...
    pub fn calculate_required_height(&self, _area: &Rect) -> u16 {
        ...
        let mut total = rows as u16 + BORDER_LINES;

        if let Some(popup) = &self.command_popup {
            total += popup.calculate_required_height(_area);
        }
