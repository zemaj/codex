...
/// When there is already a file search in progress, we debounce new requests
/// within this time window. When the timer expires, we kick off a new query
/// using the latest query string.
...
pub(crate) struct ChatComposer<'a> {
    ...
    /// Debounce control for file-search requests.
    last_file_search_sent: Option<(String, Instant)>,

    // Debounce helper: when a new query arrives within the debounce window we
    // spawn a background timer. The Arc flag allows cancelling the timer when
    // a fresher query supersedes it.
    debounce_flag: Option<Arc<AtomicBool>>,
}
...
impl ChatComposer<'_> {
    ...
    fn sync_file_search_popup(&mut self) {
        ...
        let query_changed = self.current_file_query.as_deref() != Some(&query);

        if query_changed {
            let now = Instant::now();

            match &mut self.last_file_search_sent {
                None => {
                    // First query – fire immediately
                    self.app_event_tx
                        .send(AppEvent::StartFileSearch(query.clone()));
                    self.last_file_search_sent = Some((query.clone(), now));
                }
                Some((_prev_q, last_time)) => {
                    // Within debounce window -> arm timer
                    let flag = Arc::new(AtomicBool::new(false));

                    // Cancel previous timer if any
                    if let Some(old) = self.debounce_flag.replace(flag.clone()) {
                    ...
                    std::thread::spawn(move || {
                        std::thread::sleep(FILE_SEARCH_DEBOUNCE);
                        if !flag.load(Ordering::Relaxed) {
                            tx.send(AppEvent::StartFileSearch(query_clone));
                        }
                    });
