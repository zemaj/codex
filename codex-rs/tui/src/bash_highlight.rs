...
mod tests {
    ...
    fn highlight_does_not_color_operators_inside_strings() {
        ...
        assert!(!has_gray_ops, "found gray operator tokens inside quoted regex");
        ...
        assert!(has_green_with_pipe, "expected quoted regex to be highlighted as a green string");
    }
    ...
    fn highlight_colors_command_and_flags() {
        ...
        assert!(has_short_flag && has_long_flag, "expected flags to be yellow");
    }
    ...
    fn highlight_colors_numbers() {
        ...
        let has_cyan_numbers = line.spans.iter().any(|s| {
            s.style.fg == Some(Color::Cyan) && (s.content.contains("123") || s.content.contains("456"))
        });
    ...
    fn highlight_colors_operators_outside_strings() {
        ...
        assert!(has_gray_ops, "expected operators outside strings to be gray");
    }
    ...
    fn highlight_multiple_quoted_strings() {
        ...
        let green_spans_with_pipes = line
            ...
            .count();
        assert!(green_spans_with_pipes >= 2, "expected both quoted strings to be green with pipes");

        let has_gray_pipes = line
            .spans
            .iter()
            .any(|s| s.style.fg == Some(Color::Gray) && s.content.contains("|"));
        assert!(!has_gray_pipes, "should not color '|' as operator inside quotes");
    }
}
