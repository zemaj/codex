            ...
            match mode {
                ...
                ParseMode::Lenient => {
                    match lines.as_slice() {
                        [first, .., last] => {
                            // If we are in lenient mode, we check if the first
                            // line starts with `<<'EOF'` and the last line ends
                            // with `EOF`. There must be at least 4 lines total
                            // because the heredoc markers take up 2 lines
                            // and the patch text must have at least 2 lines.
