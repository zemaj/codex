...
use code_exec::exec_events::AssistantMessageItem;
use code_exec::exec_events::CommandExecutionItem;
use code_exec::exec_events::CommandExecutionStatus;
use code_exec::exec_events::ItemCompletedEvent;
use code_exec::exec_events::ItemStartedEvent;
use code_exec::exec_events::ItemUpdatedEvent;
use code_exec::exec_events::McpToolCallItem;
use code_exec::exec_events::McpToolCallStatus;
use code_exec::exec_events::PatchApplyStatus;
use code_exec::exec_events::PatchChangeKind;
use code_exec::exec_events::ReasoningItem;
use code_exec::exec_events::ThreadErrorEvent;
use code_exec::exec_events::ThreadEvent;
use code_exec::exec_events::ThreadItem;
use code_exec::exec_events::ThreadItemDetails;
use code_exec::exec_events::ThreadStartedEvent;
use code_exec::exec_events::TodoItem as ExecTodoItem;
use code_exec::exec_events::TodoListItem as ExecTodoListItem;
use code_exec::exec_events::TurnCompletedEvent;
use code_exec::exec_events::TurnFailedEvent;
use code_exec::exec_events::TurnStartedEvent;
use code_exec::exec_events::Usage;
use code_exec::exec_events::WebSearchItem;
use code_exec::experimental_event_processor_with_json_output::ExperimentalEventProcessorWithJsonOutput;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_id = code_protocol::mcp_protocol::ConversationId::from_string(
...
fn task_started_produces_turn_started_event() {
    ...
    let out = ep.collect_thread_events(&event(
        "t1",
        EventMsg::TaskStarted(code_core::protocol::TaskStartedEvent {
...
fn plan_update_emits_todo_list_started_updated_and_completed() {
    ...
    let complete = event(
        "p3",
        EventMsg::TaskComplete(code_core::protocol::TaskCompleteEvent {
...
fn error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::Error(code_core::protocol::ErrorEvent {
...
fn stream_error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::StreamError(code_core::protocol::StreamErrorEvent {
...
fn task_complete_produces_turn_completed_with_usage() {
    ...
    let usage = code_core::protocol::TokenUsage {
        input_tokens: 1200,
    ...
    let info = code_core::protocol::TokenUsageInfo {
    ...
    let token_count_event = event(
        "e1",
        EventMsg::TokenCount(code_core::protocol::TokenCountEvent {
            info: Some(info),
            rate_limits: None,
        }),
    );
