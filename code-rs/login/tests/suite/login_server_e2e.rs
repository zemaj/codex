...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let tmp = tempdir()?;
    let code_home = tmp.path().to_path_buf();
    ...
    let server_home = code_home.clone();

    let opts = ServerOptions {
        code_home: server_home,
        client_id: code_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = code_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_code_home_dir() -> Result<()> {
    non_sandbox_test!(result);
    ...
    let tmp = tempdir()?;
    let code_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_code_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        code_home: first_code_home,
        client_id: code_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
        open_browser: false,
    ...
    let second_code_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        code_home: second_code_home,
        client_id: code_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
        open_browser: false,
