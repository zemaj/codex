...
use code_core::default_client::DEFAULT_ORIGINATOR;
...
fn server_opts(code_home: &tempfile::TempDir, issuer: String) -> ServerOptions {
...
async fn device_code_login_integration_succeeds() {
    ...
    let opts = server_opts(&code_home, issuer);

    run_device_code_login(opts)
        .await
        .expect("device code login integration should succeed");

    let auth_path = get_auth_file(code_home.path());
