...
use code_core::is_session_prefix_message;
...
async fn fork_conversation_twice_drops_to_first_message() {
    ...
    code_fork1.submit(Op::GetPath).await.unwrap();
    let fork1_history = wait_for_event(&code_fork1, |ev| {
    ...
    code_fork2.submit(Op::GetPath).await.unwrap();
    let fork2_history = wait_for_event(&code_fork2, |ev| {
        matches!(ev, EventMsg::ConversationPath(_))
    })
    .await;
