...
use code_core::ContentItem;
use code_core::debug_logger::DebugLogger;
use code_core::ConversationManager;
use code_core::LocalShellAction;
use code_core::LocalShellExecAction;
use code_core::LocalShellStatus;
use code_core::ModelClient;
use code_core::ModelProviderInfo;
use code_core::OpenRouterConfig;
use code_core::OpenRouterProviderConfig;
use code_core::NewConversation;
use code_core::Prompt;
use code_core::ReasoningItemContent;
use code_core::ResponseEvent;
use code_core::ResponseItem;
...
/// Writes an `auth.json` into the provided `code_home` with the specified parameters.
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = code_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
    ...
    let prior_system = code_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let prior_item = code_protocol::models::ResponseItem::Message {
        id: None,
    ...
    let auth_manager =
        code_core::AuthManager::from_auth_for_testing(CodexAuth::from_api_key("Test API Key"));
    let NewConversation {
        conversation: codex,
        session_configured,
        ..
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "code_cli_rs");
    assert_eq!(
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut config = load_default_config_for_test(&code_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_code_auth());
    let NewConversation {
        conversation: codex,
        conversation_id,
        session_configured: _,
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager = match CodexAuth::from_code_home(code_home.path()) {
        Ok(Some(auth)) => code_core::AuthManager::from_auth_for_testing(auth),
        Ok(None) => panic!("No CodexAuth found in code_home"),
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let code_fixture = builder.build(&server).await?;
    let codex = code_fixture.codex.clone();
...
fn create_dummy_code_auth() -> CodexAuth {
