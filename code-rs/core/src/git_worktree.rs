...
pub async fn copy_uncommitted_to_worktree(src_root: &Path, worktree_path: &Path) -> Result<usize, String> {
    ...
    let cache_copied = mirror_branch_cache_directories(src_root, worktree_path).await?;
    ...
    Ok(count + cache_copied)
}

const DEFAULT_BRANCH_CACHE_DIRS: &[&str] = &["node_modules"];

fn branch_cache_copy_enabled() -> bool {
...
fn gather_branch_cache_candidates(src_root: &Path) -> Vec<PathBuf> {
    ...
    if branch_cache_copy_enabled() {
        out.extend(auto_cache_candidates(src_root));
    ...
    for path in out {
        if let Some(clean) = sanitize_relative_path(&path) {
...
async fn mirror_branch_cache_directories(src_root: &Path, worktree_path: &Path) -> Result<usize, String> {
    let mut total = 0usize;
    for rel in gather_branch_cache_candidates(src_root) {
        let src_dir = src_root.join(&rel);
        let dest_dir = worktree_path.join(&rel);
        let copied = task::spawn_blocking(move || copy_cache_path(&src_dir, &dest_dir))
            .await
            .map_err(|err| format!("failed to mirror cached directory {}: {err}", rel.display()))?
            .map_err(|err| format!("failed to mirror cached directory {}: {err}", rel.display()))?;
        total += copied;
    }
    Ok(total)
...
fn copy_cache_path(src: &Path, dst: &Path) -> Result<usize, IoError> {
    if let Some(parent) = dst.parent() {
        stdfs::create_dir_all(parent)?;
    }

    let symlink_meta = match stdfs::symlink_metadata(src) {
        Ok(meta) => meta,
        Err(err) if err.kind() == ErrorKind::NotFound => return Ok(0),
        Err(err) => return Err(err),
    };
    let resolved_meta = match stdfs::metadata(src) {
        Ok(meta) => Some(meta),
    ...
    if symlink_meta.file_type().is_symlink() {
        if let Some(resolved) = resolved_meta {
            if resolved.is_dir() {
                ...
                return copy_dir_recursive_blocking(src, dst);
            }
            if resolved.is_file() {
                remove_existing_path(dst)?;
                stdfs::copy(src, dst)?;
        ...
        recreate_symlink_blocking(src, dst)?;
        return Ok(1);
    }
...
fn auto_cache_candidates(repo_root: &Path) -> Vec<PathBuf> {
    ...
    while let Some((dir, depth)) = queue.pop_front() {
        ...
        if cargo_toml.is_file() {
            ...
            if let Some(rel) = relative_to_root(&target_dir, repo_root) {
                out.insert(rel);
            }
        }
        ...
        if package_json.is_file() {
            ...
            if let Some(rel) = relative_to_root(&node_modules, repo_root) {
                out.insert(rel);
            }

            for default in DEFAULT_BRANCH_CACHE_DIRS {
                let candidate = dir.join(default);
                if let Some(rel) = relative_to_root(&candidate, repo_root) {
                    out.insert(rel);
                }
            }
        }
...
fn recreate_symlink_blocking(src: &Path, dst: &Path) -> Result<(), IoError> {
    ...
    #[cfg(windows)]
    {
        ...
        let meta = stdfs::metadata(src);
        match meta {
            Ok(metadata) if metadata.is_dir() => symlink_dir(&target, dst)?,
            _ => symlink_file(&target, dst)?,
        }
    }
    Ok(())
}
