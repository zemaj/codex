...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // as an elicitation.
    let code_request_id = mcp_process
        .send_code_tool_call(CodexToolCallParam {
    ...
    let expected_elicitation_request = create_expected_elicitation_request(
        ...
        code_request_id.to_string(),
        params.code_event_id.clone(),
    )?;
    ...
    let code_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(code_request_id)),
    )
    .await??;
    assert_eq!(
        JSONRPCResponse {
            jsonrpc: JSONRPC_VERSION.into(),
            id: RequestId::Integer(code_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request(
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&ExecApprovalElicitRequestParams {
            ...
            code_mcp_tool_call_id,
            code_event_id,
            code_command: command,
            code_cwd: workdir.to_path_buf(),
            code_call_id: "call1234".to_string(),
        })?),
    })
}
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_code_tool_passes_base_instructions() {
    non_sandbox_test!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = code_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn code_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    create_config_toml(code_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(code_home.path()).await?;
...
fn create_config_toml(code_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = code_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
