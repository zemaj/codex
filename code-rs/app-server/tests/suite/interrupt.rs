...
async fn shell_command_interruption() -> anyhow::Result<()> {
    ...
    .await;
    create_config_toml(&code_home, server.uri())?;

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&code_home).await?;
...
fn create_config_toml(code_home: &Path, server_uri: String) -> std::io::Result<()> {
