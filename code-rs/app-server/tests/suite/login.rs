...
fn create_config_toml(code_home: &Path) -> std::io::Result<()> {
...
async fn logout_chatgpt_removes_auth() {
    let code_home = TempDir::new().unwrap_or_else(|e| panic!("create tempdir: {e}"));
    create_config_toml(code_home.path()).expect("write config.toml");
    login_with_api_key(code_home.path(), "sk-test-key").expect("seed api key");
    assert!(code_home.path().join("auth.json").exists());

    let mut mcp = McpProcess::new_with_env(code_home.path(), &[("OPENAI_API_KEY", None)])
        .await
    ...
    assert!(
        !code_home.path().join("auth.json").exists(),
...
async fn login_and_cancel_chatgpt() {
    ...
    let mut mcp = McpProcess::new(code_home.path())
        .await
        .expect("spawn mcp process");
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize())
        .await
