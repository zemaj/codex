...
async fn test_send_message_success() {
    ...
    let code_home = TempDir::new().expect("create temp dir");
    create_config_toml(code_home.path(), &server.uri()).expect("write config.toml");
...
async fn test_send_message_session_not_found() {
    ...
    let mut mcp = McpProcess::new(code_home.path()).await.expect("spawn");
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize())
        .await
        .expect("timeout")
        .expect("init");
