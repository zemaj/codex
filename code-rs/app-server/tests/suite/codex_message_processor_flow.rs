...
async fn test_code_jsonrpc_conversation_flow() {
    ...
    let code_home = tmp.path().join("code_home");
    std::fs::create_dir(&code_home).expect("create codex home dir");
    ...
    create_config_toml(&code_home, &server.uri()).expect("write config");

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&code_home).await.expect("spawn mcp");
