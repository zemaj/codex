...
use code_protocol::protocol::{RolloutItem, RolloutLine};
...
pub fn list_sessions_for_cwd(cwd: &Path, code_home: &Path) -> Vec<ResumeCandidate> {
    ...
    if let Some(index_rows) = read_dir_index(code_home, cwd) {
        for row in index_rows {
            by_path.entry(row.path.clone()).or_insert(row);
        }
    }

    for row in fallback_scan_sessions_for_cwd(cwd, code_home) {
        by_path.entry(row.path.clone()).or_insert(row);
    }
...
fn read_dir_index(code_home: &Path, cwd: &Path) -> Option<Vec<ResumeCandidate>> {
    let index_path = super_sanitize_dir_index_path(code_home, cwd);
...
fn super_sanitize_dir_index_path(code_home: &Path, cwd: &Path) -> PathBuf {
    ...
    let mut p = code_home.to_path_buf();
    p.push("sessions");
    p.push("index");
    p.push("by-dir");
...
fn fallback_scan_sessions_for_cwd(cwd: &Path, code_home: &Path) -> Vec<ResumeCandidate> {
    const MAX_RESULTS: usize = 200;
    ...
    for line in reader.lines() {
        ...
        match parsed.item {
            ...
            RolloutItem::Event(recorded) => {
                if let Some(event) = code_core::protocol::event_msg_from_protocol(&recorded.msg) {
...
fn paths_match(meta: &Path, target: &Path) -> bool {
    ...
    if let (Ok(meta_real), Ok(target_real)) = (fs::canonicalize(meta), fs::canonicalize(target)) {
        ...
        if let (Some(meta_root), Some(target_root)) = (
            code_core::git_info::resolve_root_git_project_for_trust(&meta_real)
                .and_then(|p| fs::canonicalize(p).ok()),
            code_core::git_info::resolve_root_git_project_for_trust(&target_real)
                .and_then(|p| fs::canonicalize(p).ok()),
        ) {
