...
impl AutoCoordinatorView {
    ...
    fn cli_preview_lines(model: &AutoActiveViewModel) -> Vec<Line<'static>> {
        let mut lines: Vec<Line<'static>> = Vec::new();

        if let Some(prompt) = model.cli_prompt.as_ref() {
        ...
        if let Some(context) = model.cli_context.as_ref() {
        ...
        if let Some(hint) = model.manual_hint.as_ref() {
        ...
        if model.awaiting_submission && !lines.is_empty() {
            lines.insert(
                0,
    ...
    fn body_inner_lines(model: &AutoActiveViewModel) -> u16 {
        Self::cli_preview_lines(model).len() as u16
    }

    fn body_block_height(model: &AutoActiveViewModel) -> u16 {
        let inner = Self::body_inner_lines(model);
        if inner == 0 {
            0
        } else {
            2 + inner
    ...
    fn estimated_height_active(&self, model: &AutoActiveViewModel) -> u16 {
        let header: u16 = 1;
        header.saturating_add(Self::body_block_height(model))
    }
    ...
    fn render_body(
        ...
        let lines = Self::cli_preview_lines(model);
        if lines.is_empty() {
            return;
        }
    ...
    fn draw_view(
        ...
        let body_height = Self::body_block_height(model);
